...
UndocumentedMe.prototype.billingHistory = function( callback ) {
	return this.wpcom.req.get( '/me/billing-history', function( error, response ) {
		if ( error ) {
			return callback( error );
		}

		callback( null, {
			billingHistory: response.billing_history.map( parseDate ),
			upcomingCharges: response.upcoming_charges.map( parseDate )
		} );
	} );
};
