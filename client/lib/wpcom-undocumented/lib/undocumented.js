...
var Site = require( './site' ),
	...
	i18nUtils = require( 'lib/server-i18n/utils' ),
	store = require( 'store' );
...
function restrictByOauthKeys( query ) {
	query.client_id = config( 'wpcom_calypso_oauth_client_id' );
	query.client_secret = config( 'wpcom_calypso_oauth_client_secret' );
}
...
// This is a hack to explicitly pass the locale with each API request until the
// following issue is resolved: https://github.com/Automattic/io/issues/160
Undocumented.prototype._sendRequestWithLocale = function( originalParams, fn ) {
	var locale, updatedParams;
	if ( store.get( 'wpcom_user' ) ) {
		locale = store.get( 'wpcom_user' ).localeSlug;
	} else {
		locale = 'en';
	}
...
Undocumented.prototype.fetchDomainSuggestions = function( searchQuery, functionParams, fn ) {
	...
	this.wpcom.req.get( wpcomParams, function( error, response ) {
		if ( error ) {
			return fn( error );
		}
...
/**
 * Verify username/password/email address for new signups
 *
 * @param {string} key - either email, username or password
 * @param {string} value - either an email address, username or password
 * @param {Function} fn - Function to invoke when request is complete
 */
Undocumented.prototype.validateSignupProperty = function( key, value, fn ) {
	var args;
	debug( '/signups/validation' );

	args = {
		path: '/signups/validation/' + key + '/' + encodeURIComponent( value ),
		body: { locale: i18n.getLocaleSlug() }
	};
	this.wpcom.req.get( args, fn );
};
...
Undocumented.prototype.sitesNew = function( query, fn ) {
	...
	// Set the language for the user
	query.lang_id = i18nUtils.getLanguage( localeSlug ).value;
 ...
 * @param {Function} [fn]
 */
Undocumented.prototype.fetchDomainWhois = function( domainName, fn ) {
 ...
 * @param {Function} [fn]
 */
Undocumented.prototype.updateDomainWhois = function( domainName, whois, fn ) {
...
Undocumented.prototype.uploadFile = function( params ) {
	var req = this.wpcom.req.post( {
		path: params.path,
		formData: [ [ 'files[]', params.file ] ]
	}, params.onload );
