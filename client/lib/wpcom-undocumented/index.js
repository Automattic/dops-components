...
function WPCOMPlus( token, reqHandler ) {
	...
	WPCOM.call( this, token, function( params, fn ) {
		if ( this.isTokenLoaded() ) {
			// authToken is used in wpcom-xhr-request, token is used in rest-proxy/index.php for signup flow
			// todo: update the rest-proxy to use authToken and remove token
